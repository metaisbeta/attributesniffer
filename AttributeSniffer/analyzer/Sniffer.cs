        ...
        public ProjectReport Sniff(string folderPath)
        {
            ...
            try
            {
                ...
                List<MetricResult> metricsWithoutNamespace =
                    collectedMetrics.Where(x => x.Metric != Metric.NAMESPACES_IN_CLASS.GetIdentifier()).OrderBy(metric => metric.Metric).ToList();

                List<MetricResult> namespaceMetricsPerClass = collectedMetrics.Where(x => x.Metric.Equals(Metric.NAMESPACES_IN_CLASS.GetIdentifier())).ToList();
